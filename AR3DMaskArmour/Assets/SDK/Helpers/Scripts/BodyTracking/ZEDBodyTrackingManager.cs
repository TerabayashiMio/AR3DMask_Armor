...
public class ZEDBodyTrackingManager : MonoBehaviour
{
    ...
    private void UpdateAvatarControl(SkeletonHandler handler, sl.BodyData data)
    {
        ...
        // --- すべてのボーンに対して座標補正 ---
        var animator = handler.GetAnimator();
        var bones = handler.currentHumanBodyBones;
        var parentIds = handler.currentParentIds;

        for (int i = 0; i < bones.Length; i++)
        {
            if (bones[i] == HumanBodyBones.LastBone) continue;
            int parentIdx = parentIds[i];
            if (parentIdx < 0 || bones[parentIdx] == HumanBodyBones.LastBone) continue;

            var parentT = animator.GetBoneTransform(bones[parentIdx]);
            var childT = animator.GetBoneTransform(bones[i]);
            if (parentT == null || childT == null) continue;

            // ZEDのボーン長
            float zedLength = (data.keypoint[i] - data.keypoint[parentIdx]).magnitude;
            // アバターの元のボーン長
            float avatarLength = (childT.position - parentT.position).magnitude;
            if (avatarLength < 1e-5f) continue;
            childT.localScale = Vector3.one * zedLength / avatarLength;
            //Debug.Log($"Adjusting bone length: {childT}, ZED Length: {zedLength}, Avatar Length: {avatarLength}");
        }
        // --- ここまで ---
